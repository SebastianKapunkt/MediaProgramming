...
public abstract class DatasetViewerBase {
    ...
    /**
     * Which type of database was selected
     */
    @FXML
    ...
    private void updateMeanColorAndImage(Color meanColor, BufferedImage meanImage) {
        ...
        imagePane.setImage((meanImage == null) ? null : SwingFXUtils.toFXImage(meanImage, null));
    }
    ...
    private void onOpenDirectoryClick(ActionEvent event) {
        ...
        try (DirectoryStream<Path> files = Files.newDirectoryStream(dir.toPath(), "*.{jpg,jpeg,png}")) {
            ...
            categories.forEach((key, value) -> {
                categoryToFileList.put(key, value.toArray(new Path[0]));
            });
     ...
     * @return
     */
    public abstract BufferedImage getMeanImage(Path... imageFiles);
